...
export default function ProjectCard({ project, featured = false }) {
  ...
  return (
    <div className={`gradient-bar group rounded-xl border border-[var(--border)] bg-card overflow-hidden hover:-translate-y-1.5 hover:border-accent/45 hover:shadow-[0_12px_40px_var(--card-hover-glow)] transition-all duration-400 ${cls}`}>
      {/* Featured icon panel */}
      {featured && (
        <div className="relative flex items-center justify-center p-8 bg-gradient-to-br from-accent/10 to-accent2/5 min-h-[180px]">
          ...
          {/* Inner glow on hover */}
          <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500" style={{ background: 'radial-gradient(circle at 30% 30%, var(--card-inner-glow) 0%, transparent 60%)' }} />
        </div>
      )}
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <span className={`font-mono text-[0.62rem] uppercase tracking-wider px-2 py-0.5 rounded border ${statusColors[project.status]}`}>
